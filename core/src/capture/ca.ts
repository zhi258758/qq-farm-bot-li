...
function getCaDir(dataDir: string): string {
...
function buildSubject(attrs?: any) {
  return attrs || [
...
/** 加载或创建并持久化根 CA */
function loadOrCreateRootCa(dataDir: string) {
...
/** 为指定域名签发站点证书（返回 PEM） */
function issueLeafCert(caCert: any, caKey: any, host: string) {
  const keys = forge.pki.rsa.generateKeyPair(2048);
  const cert = forge.pki.createCertificate();
  ...
  const altNames: any[] = [{ type: 2, value: host }];
...
function getSecureContextForHost(ca: any, host: string) {
...
function getCaCertDer(ca: any): Buffer {
