 ...
 * 登录 Code / openID 提取器
 *
 * 从 HTTP 请求中提取登录 code 与 openID，规则：
 * - 网关域名（*.nqf.qq.com）上的任意非空 code 直接采用；
 * - 其他抓取域名上，code 长度 >= 16 才采用；
 * - WebSocket 升级请求的 code 放宽长度要求；
...
/** 解析 HTTP 请求头（截止 \r\n\r\n） */
function parseHttpHead(buffer: Buffer | string): any {
  if (!buffer || (buffer as any).length === 0) return null;
...
/** 从 URL/目标字符串提取查询参数 */
function extractQuery(target: any): Map<string, string> {
...
/** 通配域名匹配：`*.nqf.qq.com` 同时匹配裸域与任意子域 */
function matchesHostPattern(host: any, pattern: any): boolean {
...
function matchesAnyHostPattern(host: any, patterns: any): boolean {
  return (Array.isArray(patterns) ? patterns : []).some(pattern => matchesHostPattern(host, pattern));
}

function isGatewayHost(host: any, config: any): boolean {
...
function isCaptureHost(host: any, config: any): boolean {
  return matchesAnyHostPattern(host, config?.captureHosts || []);
}

function isValidCode(code: any, { gatewayHost, isUpgrade }: any = {}): boolean {
...
function scanJsonBody(text: string) {
...
/** 从 HTTP 请求中提取登录信息 */
function extractLoginInfo({ host, head, parsedHead, body, config = {} }: any) {
